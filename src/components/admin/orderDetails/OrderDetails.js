...
function OrderDetails() {
  ...
  useEffect(() => {
    async function fetchOrderDetails() {
      try {
        const response = await axios.get(`https://server-1-27iv.onrender.com/api/orders/${orderId}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      setIsUpdating(true);
      await axios.put(`https://server-1-27iv.onrender.com/api/orders/${orderId}`, { orderStatus: updatedStatus });
      const response = await axios.get(`https://server-1-27iv.onrender.com/api/orders/${orderId}`);
      setOrder(response.data.order);
      navigate('/Orders')
